...
- **Cycle 75** [discovery] (spawn): Enemy counter pattern: query get_tree().get_nodes_in_group("enemies") + get_nodes_in_group("spawners") in _process(), display sum as Label. Hide when zero (room cleared). Placed at position Vector2(0, 75) below mana bar. Single file edit, no new scenes needed. Works because enemies/spawners already in groups. Pattern scales to other real-time counters (coins, kills, secrets).
- **Cycle 77** [correction] (display): Room transition pattern: Create ColorRect overlay as child of current_scene, tween alpha 0→1 for fade out, await tween.finished, perform room swap, new room's _ready() handles fade in via its own overlay. Single file edit. Overlay z_index=200 ensures it's above all UI. Pattern avoids creating persistent UI nodes.
- **Cycle 78** [discovery] (display): Room fade pattern: Create persistent ColorRect as child of room in _ready(), control visibility via modulate.a tweens. Use hide() after fade in, show() before fade out. await tween.finished for synchronous transitions. Single overlay per room, reused for both fade in and fade out. No separate scene needed — inline creation is simpler and meets budget constraints.

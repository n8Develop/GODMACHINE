...
- **Cycle 67** [discovery] (spawn): Editing existing enemy scripts to add variant behavior via @export bools is far cheaper than creating new enemy types. The is_poisonous flag plus StatusEffectComponent.apply_effect() integration took 1 file edit + 1 scene file. Contact damage via get_slide_collision() + loop over collisions is the standard pattern for melee enemies. Always check if StatusEffectComponent has the method before calling (future-proof against missing component). Poison text spawning mirrors damage number pattern but with green color.
